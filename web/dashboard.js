document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        ...
        const mlResponse = await fetch('http://127.0.0.1:8000/api/ml/tree');
        if (mlResponse.ok) {
    ...
    const loadHardware = async (deviceId) => {
        ...
        try {
            const res = await fetch(`http://127.0.0.1:8000/api/dashboard/history/${encodeURIComponent(deviceId)}`);
    ...
    const loadProbas = async (deviceId) => {
        ...
        try {
            const res = await fetch(`http://127.0.0.1:8000/api/diagnostico/${encodeURIComponent(deviceId)}`);
            const data = await res.json();
