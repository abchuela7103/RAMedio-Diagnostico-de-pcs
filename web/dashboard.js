document.addEventListener('DOMContentLoaded', async () => {
    ...
    const loadHardware = async (deviceId) => {
        ...
        try {
            const res = await fetch(`/api/dashboard/history/${encodeURIComponent(deviceId)}`);
            ...
            if(data.timestamps && data.timestamps.length > 0) {
                const hardwareOption = {
                    ...
                    xAxis: {
                        type: 'category',
                        boundaryGap: false,
                        data: data.timestamps.map(ts => new Date(ts).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit', second:'2-digit', hour12: false})),
